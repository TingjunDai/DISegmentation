...
import numpy as np
import math
from models.birefnet.modules.registry import HEADS
...
class DCN_V2_CONV(jt.Function):
    ...
    def grad(self, grad_output):
        input_grad, offset_grad, mask_grad, weight_grad, bias_grad = dcn_v2_conv_backward(self.input, self.offset,
                                                                                          self.mask, self.weight,
                                                                                          self.bias, grad_output,
...
def dcn_v2_pooling_forward(input, bbox, trans, spatial_scale, pooled_size, output_dim, no_trans, group_size, part_size,
...
def dcn_v2_pooling_backward(grad_output, input, bbox, trans, output_count, no_trans, spatial_scale, output_dim,
                            group_size, pooled_size, part_size, sample_per_part, trans_std):
...
class DCN_V2_POOLING(jt.Function):
    def execute(self, input, rois, offset, spatial_scale, pooled_size, output_dim, no_trans, group_size=1,
        ...
        output, output_count = dcn_v2_pooling_forward(input, rois, offset, self.spatial_scale, self.pooled_size,
...
class DCN(DCNv2):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.regular_conv = nn.Conv(in_channels, out_channels, kernel_size, stride=stride, padding=self.padding, bias=True)

    def execute(self, input):
        offset = self.offset_conv(input)
        modulator = (2.0 * jt.sigmoid(self.modulator_conv(input)))
        return dcn_v2_conv(input, offset, modulator,
...
def test_conv():
    ...
    dcn = DCN(64, 64, kernel_size=(1, 1), stride=(1, 1),
