...
from models.birefnet.modules.decoder_blocks import BasicDecBlk, ResBlk, HierarAttDecBlk
from models.birefnet.modules.lateral_blocks import BasicLatBlk
...
class BiRefNet(nn.Module):
    ...
    def laplacian(self, x, kernel_size=5):
        ...
        for b in range(batch_size):
            for c in range(channels):
                channel = x_np[b, c]
                laplacian_result = cv2.Laplacian(channel, cv2.CV_32F, ksize=kernel_size)
                laplacian_features.append(laplacian_result)

        laplacian_features = np.array(laplacian_features).reshape(batch_size, channels, height, width)
    ...
    def forward_ori(self, x):
        ...
        if self.training and self.config.out_ref:
            features.append(self.laplacian(jt.mean(x, dim=1).unsqueeze(1), kernel_size=5))
