...
from models.udun.modules.DCM import DCM
from models.udun.modules.StrucDe import StrucDe
from models.udun.modules.TrunkDe import TrunkDe
from models.udun.modules.UnionDe import UnionDe
...
class UDUN(nn.Module):
    def __init__(self, bb_pretrained=True):
        ...
        if bb_pretrained:
            self.bkbone.load_state_dict(jt.load(config.weights['resnet50']))

    def execute(self, x):
        y = nn.interpolate(x, size=(config.size[0]//4, config.size[1]//4), mode='bilinear', align_corners=True)
