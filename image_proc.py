...
def preproc(image, label, trunk=None, struct=None, preproc_methods=['flip']):
    if 'flip' in preproc_methods:
        if trunk == None:
            image, label = cv_random_flip(image, label)
        else:
            image, label, trunk, struct = cv_random_flip(image, label, trunk, struct)
    if 'crop' in preproc_methods:
        if trunk == None:
            image, label = random_crop(image, label)
        else:
            image, label, trunk, struct = random_crop(image, label, trunk, struct)
    ...
    return (image, label) if trunk == None else (image, label, trunk, struct)
...
def cv_random_flip(img, label, trunk=None, struct=None):
    ...
    return (img, label) if trunk == None else (img, label, trunk, struct)


def random_crop(image, label, trunk=None, struct=None):
    border = 30
    ...
    return (image.crop(random_region), label.crop(random_region)) if trunk == None else (image.crop(random_region), label.crop(random_region), trunk.crop(random_region), struct.crop(random_region))
